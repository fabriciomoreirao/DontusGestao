...
function ScheduleCalendar({ view, anchor, commitments, specialEvents, onPrevious, onNext, onOpenDay, onOpenCommitment }: { view: "week" | "day"; anchor: Date; commitments: AgendaCommitment[]; specialEvents: AgendaSpecialEvent[]; onPrevious: () => void; onNext: () => void; onOpenDay: (date: string) => void; onOpenCommitment: (entry: AgendaCommitment) => void }) {
  ...
  const label = view === "day"
    ? new Intl.DateTimeFormat("pt-BR", { weekday: "long", day: "2-digit", month: "long", year: "numeric" }).format(days[0])
  ...
  return <section className={`agenda-calendar-card agenda-schedule-card ${view}`}><header className="agenda-calendar-title"><button className="icon-button" onClick={onPrevious} aria-label="Período anterior"><ChevronLeft size={19} /></button><h2>{label}</h2><button className="icon-button" onClick={onNext} aria-label="Próximo período"><ChevronRight size={19} /></button></header><div className="agenda-schedule-grid">{days.map((day) => { const key=dayKey(day); const entries=commitments.filter(entry=>dateKey(entry.startsAt)===key).sort((a,b)=>a.startsAt.localeCompare(b.startsAt)); const commemorations=specialEvents.filter(entry=>entry.date===key); return <article key={key} className={key===dayKey(new Date())?"today":""}><header><button type="button" onClick={()=>onOpenDay(key)}><strong>{day.toLocaleDateString("pt-BR",{weekday:"short"})}</strong><b>{day.getDate()}</b><small>{day.toLocaleDateString("pt-BR",{month:"short"})}</small></button></header><div className="agenda-schedule-events">{commemorations.map(entry=><div className="agenda-schedule-event special" key={entry.id} style={{"--event-color":entry.color} as CSSProperties}><span>{entry.title}</span><small>{entry.detail}</small></div>)}{entries.map(entry=><button type="button" className="agenda-schedule-event" key={entry.id} style={{"--event-color":entry.statusColor} as CSSProperties} onClick={()=>onOpenCommitment(entry)}><time>{time(entry.startsAt)} – {time(entry.endsAt)}</time><span>{entry.title}</span><small>{entry.responsibleName} · {entry.statusName}</small></button>)}{entries.length===0&&commemorations.length===0&&<button type="button" className="agenda-schedule-empty" onClick={()=>onOpenDay(key)}>Nenhum compromisso</button>}</div></article>;})}</div></section>;
}

function DayCommitmentsModal({ date, commitments, statuses, calendars, busy, onClose, onOpenCommitment, onChangeStatus }: { date: string; commitments: AgendaCommitment[]; statuses: AgendaStatus[]; calendars: AgendaCalendar[]; busy: boolean; onClose: () => void; onOpenCommitment: (entry: AgendaCommitment) => void; onChangeStatus: (commitmentId: string, statusId: string) => Promise<boolean> }) {
  const ordered = useMemo(() => [...commitments].sort((a, b) => a.startsAt.localeCompare(b.startsAt)), [commitments]);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [statusId, setStatusId] = useState("");
  const [saving, setSaving] = useState(false);
  const formattedDate = new Intl.DateTimeFormat("pt-BR", { weekday: "long", day: "2-digit", month: "long", year: "numeric" }).format(new Date(`${date}T12:00:00`));
  const startEditing = (entry: AgendaCommitment) => {
    setEditingId(entry.id);
    setStatusId(entry.agendaStatusId);
  };
  const saveStatus = async (entry: AgendaCommitment) => {
    if (!statusId || statusId === entry.agendaStatusId) { setEditingId(null); return; }
    setSaving(true);
    const saved = await onChangeStatus(entry.id, statusId);
    setSaving(false);
    if (saved) setEditingId(null);
  };
  return <AgendaModal title="Compromissos do dia" className="agenda-day-modal" onClose={onClose}>
    <section className="agenda-day-summary">
      <header><span className="agenda-day-summary-icon"><CalendarDays size={18} /></span><span><strong>{formattedDate}</strong><small>{ordered.length} {ordered.length === 1 ? "compromisso" : "compromissos"} no dia</small></span></header>
      <div className="agenda-day-list">{ordered.length === 0 ? <EmptyAgenda text="Nenhum compromisso encontrado neste dia." /> : ordered.map((entry) => {
        const editing = editingId === entry.id;
        const calendar = calendars.find((item) => item.id === entry.agendaId);
        return <article key={entry.id} className={editing ? "editing" : ""} style={{ "--event-color": entry.statusColor } as CSSProperties}>
          <button type="button" className="agenda-day-entry" onClick={() => editing ? setEditingId(null) : startEditing(entry)} aria-expanded={editing}>
            <span className="agenda-day-entry-time"><strong>{time(entry.startsAt)}</strong><small>{time(entry.endsAt)}</small></span>
            <span className="agenda-day-entry-copy"><strong>{entry.title}</strong><small>{entry.responsibleName}{calendar ? ` · ${calendar.departmentName || calendar.name}` : ""}</small></span>
            <span className="agenda-day-entry-status" style={{ "--status-color": entry.statusColor } as CSSProperties}><i />{entry.statusName || "Sem status"}</span>
            <ChevronRight size={17} />
          </button>
          {editing && <div className="agenda-day-status-editor"><label><span>Editar status</span><select value={statusId} disabled={busy || saving} onChange={(event) => setStatusId(event.target.value)}>{statuses.filter((status) => status.active || status.id === entry.agendaStatusId).map((status) => <option value={status.id} key={status.id}>{status.name}</option>)}</select></label><div><button type="button" className="secondary-button compact" disabled={busy || saving} onClick={() => onOpenCommitment(entry)}><Edit3 size={15} /> Abrir detalhes</button><button type="button" className="primary-button compact" disabled={busy || saving || !statusId} onClick={() => void saveStatus(entry)}><Save size={15} /> {saving ? "Salvando..." : "Salvar status"}</button></div></div>}
        </article>;
      })}</div>
    </section>
  </AgendaModal>;
...
function AgendaModal({ title, onClose, headerAction, className = "", children }: { title: string; onClose: () => void; headerAction?: React.ReactNode; className?: string; children: React.ReactNode }) { return <div className="modal-backdrop" onMouseDown={(event) => event.currentTarget === event.target && onClose()}><div className={`modal agenda-modal ${className}`.trim()} role="dialog" aria-modal="true" aria-label={title}><div className="modal-head"><div><span className="eyebrow">AGENDA</span><h2>{title}</h2></div>{headerAction}<button className="icon-button" onClick={onClose} aria-label="Fechar"><X size={20} /></button></div>{children}</div></div>; }
