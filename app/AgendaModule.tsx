...
export default function AgendaModule({ module, busy, operate, currentEmail, onOpenSettings }: { module: AgendaModuleData; busy: boolean; operate: Operate; currentEmail: string; onOpenSettings?: () => void }) {
  ...
  return <>
    ...
    <MonthCalendar month={month} commitments={filtered} specialEvents={specialEventsForCalendar(month, module.celebrants ?? module.collaborators)} onPrevious={() => setMonth((value) => new Date(value.getFullYear(), value.getMonth() - 1, 1))} onNext={() => setMonth((value) => new Date(value.getFullYear(), value.getMonth() + 1, 1))} onOpenDay={setSelectedDay} />
