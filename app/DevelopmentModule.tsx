...
export default function DevelopmentModule({ items, employees, user, canEdit, canCreate, busy, operate }: Props) {
  ...
  const remove = async (item: WorkItem) => {
    if (!window.confirm(`Excluir a demanda “${item.title}”?`)) return false;
  ...
  const canDelete = user.isCoordinator || /admin|gestor|coordenador/i.test(user.role);
  ...
  const saveRelease = async (title: string, plannedAt: string, notes: string) => {
    ...
    const result = current
      ? await operate({ action: "updateWorkItem", id: current.id, title, owner: current.owner, customerName: current.customer_name, amountCents: current.amount_cents, version: current.version, description: JSON.stringify({ ...detail, bugReports: readRelease(current.description)?.bugReports ?? [] }) }, "Versão atualizada com sucesso.")
  ...
  return <section className="development-module">
    <header className="development-heading">
      <div><span className="eyebrow">OPERAÇÃO · TI</span><h1>Desenvolvimento</h1><p>Triagem, execução, correções e versões em uma visão única.</p></div>
      {canCreate && <button className="primary-button" onClick={() => setCreating(true)}><Plus size={17} /> Nova demanda</button>}
    </header>
    ...
    {tab === "triage" && <TriageView items={records} view={view} canEdit={canEdit} canDelete={canDelete} onOpen={setSelected} onDelete={remove} />}
    {tab === "process" && <ProcessView items={processRecords} view={view} canEdit={canEdit} canDelete={canDelete} onOpen={setSelected} onDelete={remove} />}
...
function DevelopmentCard({ item, canEdit, canDelete, onOpen, onDelete }: { item: WorkItem; canEdit: boolean; canDelete: boolean; onOpen: () => void; onDelete: () => void }) {
  ...
  return <article className="development-card-shell"><button className="development-card" onClick={onOpen}><span className="development-card-code">{detail.sourceProtocol || item.id.slice(0, 8)}</span><strong>{item.title}</strong><small>{item.customer_name || "Demanda interna"}</small><footer><span>{detail.processStage}</span><em>{daysInStage} dia(s) na etapa</em></footer></button>{(canEdit || canDelete) && <span className="development-card-actions">{canEdit && <button type="button" onClick={onOpen} title="Editar detalhes" aria-label="Editar detalhes"><Pencil size={14} /></button>}{canDelete && <button type="button" className="delete" onClick={onDelete} title="Excluir demanda" aria-label="Excluir demanda"><Trash2 size={14} /></button>}</span>}</article>;
}

function TriageView({ items, view, canEdit, canDelete, onOpen, onDelete }: { items: WorkItem[]; view: "list" | "kanban"; canEdit: boolean; canDelete: boolean; onOpen: (item: WorkItem) => void; onDelete: (item: WorkItem) => void }) {
  ...
  if (view === "kanban") return <section className="development-board"><DevelopmentColumn title="Recebida" items={receivedItems} canEdit={canEdit} canDelete={canDelete} onOpen={onOpen} onDelete={onDelete} /><DevelopmentColumn title="Em análise" items={reviewItems} canEdit={canEdit} canDelete={canDelete} onOpen={onOpen} onDelete={onDelete} /></section>;
...
function ProcessView({ items, view, canEdit, canDelete, onOpen, onDelete }: { items: WorkItem[]; view: "list" | "kanban"; canEdit: boolean; canDelete: boolean; onOpen: (item: WorkItem) => void; onDelete: (item: WorkItem) => void }) {
  if (view === "kanban") {
    ...
    return <section className="development-board process-board">{processColumns.map((column) => <DevelopmentColumn key={column.name} title={column.name} tone={column.tone} items={activeBoard.filter((item) => readDevelopment(item.description)?.processStage === column.name)} canEdit={canEdit} canDelete={canDelete} onOpen={onOpen} onDelete={onDelete} />)}</section>;
  }
...
function VersionView({ items, releases, canCreate, canEdit, canDelete, onCreate, onEdit, onDelete, onOpen }: { items: WorkItem[]; releases: WorkItem[]; canCreate: boolean; canEdit: boolean; canDelete: boolean; onCreate: () => void; onEdit: (item: WorkItem) => void; onDelete: (item: WorkItem) => void; onOpen: (item: WorkItem) => void }) {
  return <section className="version-list panel"><header><div><span className="eyebrow">LIBERAÇÕES</span><h2>Versões em validação</h2></div><p>Cadastre a versão e encaminhe os pontos para teste.</p>{canCreate && <button className="primary-button" onClick={onCreate}><Plus size={16} /> Nova versão</button>}</header>{releases.length === 0 ? <Empty text="Cadastre a primeira versão para encaminhar demandas." /> : releases.map((release) => { const detail = readRelease(release.description)!; const related = items.filter((item) => readDevelopment(item.description)?.versionName === release.title); return <article className="version-release" key={release.id}><div className="version-release-head"><span className="version-mark"><GitBranch size={17} /></span><span><b>{release.title}</b><small>Previsão: {detail.plannedAt ? displayDate(detail.plannedAt) : "não informada"} · {related.length} tarefa(s)</small></span><em>{detail.testedBy ? `Testada por ${detail.testedBy}` : "Aguardando testes"}</em><span className="development-inline-actions">{canEdit && <button type="button" onClick={() => onEdit(release)} title="Editar versão"><Pencil size={14} /></button>}{canDelete && <button type="button" className="delete" onClick={() => onDelete(release)} title="Excluir versão"><Trash2 size={14} /></button>}</span></div>{detail.notes && <p>{detail.notes}</p>}<div className="version-release-tasks">{related.length === 0 ? <small>Nenhuma tarefa encaminhada ainda.</small> : related.map((item) => <div className="version-row-shell" key={item.id}><button className="version-row" onClick={() => onOpen(item)}><span><b>{item.title}</b><small>{readDevelopment(item.description)?.sourceProtocol || "Demanda TI"}</small></span><span>{readDevelopment(item.description)?.versionState || "Aguardando validação"}</span><ChevronRight size={17} /></button><span className="development-inline-actions">{canEdit && <button type="button" onClick={() => onOpen(item)} title="Editar demanda"><Pencil size={14} /></button>}{canDelete && <button type="button" className="delete" onClick={() => onDelete(item)} title="Excluir demanda"><Trash2 size={14} /></button>}</span></div>)}</div></article>; })}</section>;
}
...
function DevelopmentColumn({ title, items, tone, canEdit = false, canDelete = false, onOpen, onDelete = () => undefined }: { title: string; items: WorkItem[]; tone?: string; canEdit?: boolean; canDelete?: boolean; onOpen: (item: WorkItem) => void; onDelete?: (item: WorkItem) => void }) {
  const [limit, setLimit] = useState(10);
  const visibleItems = items.slice(0, limit);
  return <section className={`development-column ${tone ?? ""}`}><header><span>{title}</span><b>{items.length}</b></header><div>{items.length ? visibleItems.map((item) => <DevelopmentCard item={item} canEdit={canEdit} canDelete={canDelete} onOpen={() => onOpen(item)} onDelete={() => void onDelete(item)} key={item.id} />) : <p>Nenhuma tarefa nesta etapa.</p>}{items.length > visibleItems.length && <button type="button" className="development-column-more" onClick={() => setLimit((value) => value + 10)}>Ver mais ({items.length - visibleItems.length})</button>}</div></section>;
}
