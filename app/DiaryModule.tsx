...
import { CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, ClipboardList, Plus, Save, X } from "lucide-react";
...
export default function DiaryModule({ module, busy, operate }: { module: DiaryModuleData; busy: boolean; operate: Operate }) {
  ...
  return <>
    <div className="page-header diary-header"><div><span className="eyebrow">MÓDULO · ROTINA DIÁRIA</span><h1><ClipboardList size={22} /> Diário de Bordo</h1><p>Suas atividades manuais e compromissos da agenda sob sua responsabilidade, em um único lugar.</p></div><button className="primary-button" onClick={() => setCreating(true)}><Plus size={17} /> Nova atividade</button></div>
    ...
    <div className="diary-columns">
      <DiaryColumn title={`Pendentes (${pending.length})`} tone="pending" entries={pending} module={module} busy={busy} onChangeStatus={changeStatus} empty="Tudo em dia! Nenhuma atividade pendente." />
      <DiaryColumn title={`Finalizados (${finished.length})`} tone="done" entries={finished} module={module} busy={busy} onChangeStatus={changeStatus} empty="Nenhuma atividade concluída neste dia." />
    </div>
    {cancelled.length > 0 && <section className="diary-cancelled"><h2>Cancelados ({cancelled.length})</h2>{cancelled.map((entry) => <DiaryEntryCard key={entry.id} entry={entry} module={module} busy={busy} onChangeStatus={changeStatus} />)}</section>}
...
function DiaryColumn({ title, tone, entries, module, busy, onChangeStatus, empty }: { title: string; tone: "pending" | "done"; entries: DiaryEntry[]; module: DiaryModuleData; busy: boolean; onChangeStatus: (entry: DiaryEntry, value: string) => Promise<void>; empty: string }) {
  return <section className={`diary-column ${tone}`}><h2>{title}</h2>{entries.length === 0 ? <div className="diary-empty"><CheckCircle2 size={25} /><p>{empty}</p></div> : entries.map((entry) => <DiaryEntryCard key={entry.id} entry={entry} module={module} busy={busy} onChangeStatus={onChangeStatus} />)}</section>;
}

function DiaryEntryCard({ entry, module, busy, onChangeStatus }: { entry: DiaryEntry; module: DiaryModuleData; busy: boolean; onChangeStatus: (entry: DiaryEntry, value: string) => Promise<void> }) {
  ...
  return <article className="diary-entry" style={{ "--diary-color": entry.statusColor } as CSSProperties}><span className="diary-entry-icon"><CalendarDays size={16} /></span><div className="diary-entry-content"><strong>{entry.title}</strong><small>{entry.source === "agenda" ? "Compromisso da Agenda" : entry.type} · {time(entry.occursAt)}</small>{entry.description && <p>{entry.description}</p>}</div><div className="diary-entry-status"><span>{entry.status}</span><select aria-label={`Alterar status de ${entry.title}`} value={value} disabled={busy} onChange={(event) => void onChangeStatus(entry, event.target.value)}>{statuses.map((status) => <option key={status.id} value={status.id}>{status.name}</option>)}</select></div></article>;
}

function ManualActivityModal({ date, busy, onClose, onSave }: { date: Date; busy: boolean; onClose: () => void; onSave: (payload: { title: string; type: string; description: string; dueAt: string }) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.currentTarget === event.target && onClose()}><div className="modal diary-modal" role="dialog" aria-modal="true" aria-label="Nova atividade manual"><div className="modal-head"><div><span className="eyebrow">DIÁRIO DE BORDO</span><h2>Nova atividade manual</h2><p>Registrada para {dateLabel(date)}.</p></div><button className="icon-button" onClick={onClose} aria-label="Fechar"><X size={20} /></button></div><form className="form-grid" onSubmit={submit}><label className="wide">Título *<input name="title" required placeholder="O que precisa ser feito?" autoFocus /></label><label className="wide">Tipo<select name="type" defaultValue="Outro"><option>Outro</option><option>Tarefa</option><option>Lembrete</option><option>Retorno</option><option>Estudo</option></select></label><label className="wide">Descrição<textarea name="description" rows={4} placeholder="Detalhes opcionais..." /></label><div className="form-actions wide"><button type="button" onClick={onClose}>Cancelar</button><button className="primary-button" type="submit" disabled={busy}><Save size={16} /> {busy ? "Criando..." : "Criar"}</button></div></form></div></div>;
}
