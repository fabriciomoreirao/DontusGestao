...
export default function NoticesModule({ module, busy, operate }: { module: NoticesModuleData; busy: boolean; operate: Operate }) {
  ...
  return <div className="notices-module">
    <header className="notices-page-header"><div className="notices-title-icon"><Megaphone size={21} /></div><div><span className="eyebrow">COMUNICAÇÃO INTERNA</span><h1>Avisos</h1><p>Comunicados, eventos e informações importantes para você.</p></div><span className="notices-unread-counter"><Bell size={15} /> {unread.length} pendente(s)</span></header>
...
export function NoticesAdmin({ module, employees, busy, operate }: { module: NoticesModuleData; employees: NoticeEmployee[]; busy: boolean; operate: Operate }) {
  ...
  return <div className="notice-admin-module">
    <header className="notices-page-header"><div className="notices-title-icon"><Megaphone size={21} /></div><div><span className="eyebrow">ADMINISTRAÇÃO</span><h1>Gerenciar avisos</h1><p>Publique comunicados ou eventos para a empresa ou para um colaborador.</p></div><button className="primary-button" onClick={() => setEditing(null)}><Plus size={17} /> Novo aviso</button></header>
