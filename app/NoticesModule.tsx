...
export function NoticesAdmin({ module, employees, busy, operate }: { module: NoticesModuleData; employees: NoticeEmployee[]; busy: boolean; operate: Operate }) {
  ...
  return <div className="notice-admin-module">
    ...
    {editing !== undefined && <NoticeModal notice={editing} employees={employees} busy={busy} onClose={() => setEditing(undefined)} onSave={async (payload) => { const result = await operate({ action: "saveNotice", ...(editing ? { requireConfirmation: true } : {}), id: editing?.id, ...payload }, editing ? "Aviso atualizado com sucesso." : "Aviso publicado com sucesso."); if (result) setEditing(undefined); }} />}
