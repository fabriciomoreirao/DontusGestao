...
function PasswordRecoveryModal({ initialEmail, onClose }: { initialEmail: string; onClose: () => void }) {
  ...
  return <ModalShell title="Recuperar acesso" subtitle="Confirme o endereço cadastrado para receber uma nova senha temporária." eyebrow="SEGURANÇA DA CONTA" className="password-recovery-modal" onClose={onClose}>
    {sent ? <div className="password-recovery-success"><span className="password-recovery-success-icon"><CheckCircle2 size={30} /></span><strong>Nova senha enviada</strong><p>A senha temporária foi direcionada para <b>{maskedEmail}</b>. Use-a no próximo acesso e depois altere-a no menu da sua conta.</p><div className="form-actions"><button className="primary-button" type="button" onClick={onClose}>Voltar ao login</button></div></div> : <form className="form-grid password-recovery-form" onSubmit={submit}><div className="password-recovery-address wide"><span className="password-recovery-mail-icon"><Mail/></span><span><small>E-mail cadastrado</small><strong>{maskedEmail||"E-mail não informado"}</strong><p>Por segurança, somente o início e o final do endereço são exibidos.</p></span></div><div className="password-recovery-notice wide"><ShieldCheck size={17}/><p>Ao continuar, a senha atual será substituída por uma senha temporária enviada ao e-mail acima.</p></div>{!email&&<p className="form-error wide">Informe seu e-mail na tela de acesso antes de solicitar uma nova senha.</p>}{error && <p className="form-error wide">{error}</p>}<div className="form-actions wide"><button type="button" onClick={onClose}>Cancelar</button><button className="primary-button" disabled={busy||!email} type="submit">{busy ? "Enviando..." : "Enviar nova senha"}</button></div></form>}
  </ModalShell>;
}
...
function SupportAccessView({ access, onOpen }: { access: AccessManagement | null; onOpen: (name: string) => void }) {
  ...
  const [submittedFilters, setSubmittedFilters] = useState({ identification: "", name: "", type: "" });
  const submitFilters = () => setSubmittedFilters({ identification: query, name: nameQuery, type: accessType });
  const entries = (access?.employees ?? []).filter((employee) => {
    const identification = submittedFilters.identification.trim().toLocaleLowerCase("pt-BR");
    const name = submittedFilters.name.trim().toLocaleLowerCase("pt-BR");
    const type = employee.isCoordinator ? "coordinator" : "collaborator";
    const username = employee.email.split("@")[0];
    return (!identification || `${username} ${employee.email}`.toLocaleLowerCase("pt-BR").includes(identification))
      && (!name || employee.displayName.toLocaleLowerCase("pt-BR").includes(name))
      && (!submittedFilters.type || submittedFilters.type === type);
  });
  return <section className="support-access-page">
    ...
    <div className="support-access-panel">
      ...
      <div className="support-access-search"><label>Identificação<input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") submitFilters(); }} placeholder="Usuário ou e-mail" autoFocus /></label><label>Nome<input value={nameQuery} onChange={(event) => setNameQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") submitFilters(); }} placeholder="Nome do colaborador" /></label><label>Tipo<select value={accessType} onChange={(event) => setAccessType(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") submitFilters(); }}><option value="">Todos os tipos</option><option value="coordinator">Coordenador</option><option value="collaborator">Colaborador</option></select></label><button className="primary-button" type="button" onClick={submitFilters}><Search size={16} /> Pesquisar</button></div>
...
function ModalShell({ title, subtitle, eyebrow = "NOVO REGISTRO", className = "", onClose, children }: { title: string; subtitle: string; eyebrow?: string; className?: string; onClose: () => void; children: React.ReactNode }) {
  return <div className="modal-backdrop standard-modal-backdrop" onMouseDown={(event) => event.currentTarget === event.target && onClose()}><div className={`modal standard-modal ${className}`.trim()} role="dialog" aria-modal="true" aria-label={title}><div className="modal-head"><div><span className="eyebrow">{eyebrow}</span><h2>{title}</h2><p>{subtitle}</p></div><button className="icon-button" onClick={onClose} aria-label="Fechar"><X size={20} /></button></div>{children}</div></div>;
}
