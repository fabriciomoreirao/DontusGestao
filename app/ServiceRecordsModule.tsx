...
import {
  CalendarDays, CheckCircle2, ChevronDown, Clock3, Headphones, Layers3, Pencil,
...
type Employee = { id: string; displayName: string; departmentId: string | null; departmentName: string; departmentNames: string[]; active: boolean };
...
export default function ServiceRecordsModule({ items, customers, catalogs, employees, departments, currentUser, busy, canCreate, canEdit, canDelete, operate, onOpenSettings }: {
  ...
  return <section className="service-records-module">
    <header className="service-records-header"><div><span className="service-title-icon"><Headphones size={20} /></span><span><h1>Atendimentos</h1><p>Registros de atendimento ao cliente, organizados por setor.</p></span></div><div><button className="icon-button" onClick={onOpenSettings} aria-label="Configurar atendimentos" title="Configurar atendimentos"><Settings size={18} /></button>{canCreate && <><button className="secondary-button" onClick={() => setModal("bulk")}><Layers3 size={16} /> Inserção em massa</button><button className="primary-button" onClick={() => setModal("single")}><Plus size={16} /> Inserir atendimento</button></>}</div></header>
...
function ServiceEntryModal({ mode, editing, currentUser, customers, catalogs, employees, departments, busy, onClose, onSave }: { mode: "single" | "bulk"; editing: WorkItem | null; currentUser: CurrentUser; customers: Customer[]; catalogs: Catalog[]; employees: Employee[]; departments: Department[]; busy: boolean; onClose: () => void; onSave: (rows: EntryRow[]) => Promise<void> }) {
  ...
  const tools = options("serviceTool");
